...
class MarketData:
    ...
    def _process_etf_data(self, parameter):
        ...
        for ticker in etf_tickers:
            if parameter not in etf_metadata_dict[ticker]:
                skipped_due_to_missing.append(ticker)
                continue
            ...
            if not df.empty:
                ...
                skipped_due_to_empty.append(ticker)

        if skipped_due_to_missing:
            print(f"❌ '{parameter}' key not found for: {', '.join(skipped_due_to_missing)}")

        if skipped_due_to_empty:
            print(f"⚠️ '{parameter}' is present but empty for: {', '.join(skipped_due_to_empty)}")
